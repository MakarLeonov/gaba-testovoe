...
Я положил .env файл в репозиторий, потому что это тестовое задание
Обычно я не храню файлы окружения в репах
...
`DATABASE_URL` - url для соединения Prisma
`PORT` - порт сервера
...
Успешный запрос: `200`
Промокод не найден: `404`
Промокод просрочен: `410` | `Promocode has expired`
Превышение лимита: `409` | `Activation limit reached`
Повторное использование одинаковой почты: `409` | `Already activated by this email`
